...

{
    ...
    void Start()
    {
        ...
        PlayerResource.Instance.time += PlayerResource.Instance.id;
    }
    ...
    void Update()
    {
        if (PlayerResource.Instance.GameIsPaused == false && PlayerResource.Instance.gameMode == "timetrial" && PlayerResource.Instance.starttimer == true) //таймер отсчитывает назад в режиме игры на время и когда нет паузы
        {
            ...
            timeMin = Math.Floor((PlayerResource.Instance.time - PlayerResource.Instance.id) / 60); //получаем целые минуты, округленные вниз до целого
            timeSec = Math.Floor((PlayerResource.Instance.time - PlayerResource.Instance.id) - (timeMin * 60)); //целые секунды, округленные вниз до целого
            if (timeSec > 9)
            {
            ...
            if ((PlayerResource.Instance.time - PlayerResource.Instance.id) <= 0) //если время вышло
            {
